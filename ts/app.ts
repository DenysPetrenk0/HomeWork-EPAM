const tickets = (persons:Array<number>):string => {
  let twentyFiveDollar:number = 0;
  let fiftyDollars:number = 0;
...
const getSum = (number1:string, number2:string):string => {
  let sum = [];
  let increment = 0;
  let array1:Array<string> = number1.toString().split("");
  let array2:Array<string> = number2.toString().split("");

    while (array1.length > 0 || array2.length > 0) {
      
    let number:number =  parseInt(array1.pop() || "0")+ parseInt(array2.pop() || "0");
...
const getQuantityPostsByAuthor = (listOfPosts:Array<any>, authorName:string):string => {
  let post = 0;
  let comm = 0;
  listOfPosts.forEach((item:any) => {
